...
extension FileHandle {
    ...
    func write(line: String) {
        do {
            ...
            Logger.wynKit.info("Failed to write line: \(error)")
        }
    }
    ...
    func writeApplicaitonInfo() {
        ...
        header += "Wyn Version: \(Bundle.main.infoDictionary?["CFBundleShortVersionString"] ?? "")\n"
    ...
    func writeInfo(for bottle: Bottle) {
        ...
        if let version = WynWineInstaller.wynWineVersion() {
            header += "WynWine Version: \(version.major).\(version.minor).\(version.patch)\n"
        }
        ...
        header += "Translation Layer: \(bottle.settings.translationLayer.rawValue)\n"
        header += "DXVK flag: \(bottle.settings.dxvk)\n"
