...
extension FileManager {
    ...
    func replaceFile(at originalURL: URL, with replacementURL: URL, makeOriginalCopy: Bool = true) throws {
        if fileExists(atPath: originalURL.path(percentEncoded: false)) {
            ...
            try createDirectory(at: originalURL.deletingLastPathComponent(), withIntermediateDirectories: true)
            try copyItem(at: replacementURL, to: originalURL)
        }
    }

    func installFileIfContentDiffers(at destination: URL, from source: URL) throws {
        if fileExists(atPath: destination.path(percentEncoded: false)) {
